...
버전: **0.1.2** (최신) / 0.1.1 / 0.1.0  
날짜: **2026-09-04**

이 문서는 `PLAN.md` 1차 구현, `PLAN2.md` 검색·필터, 그리고 설치/포터블 패키징까지 정리한다.
...
산출물은 `release/` 폴더에 있다. Windows는 CPU에 맞는 파일을 고른다.
...
| 파일 | 용도 | 크기 |
|---|---|---|
| `AXGATE-Log-Viewer-Setup-0.1.2-win-x64.exe` | Windows 설치 프로그램 (Intel/AMD 64비트) | 73MB |
| `AXGATE-Log-Viewer-0.1.2-portable-win-x64.exe` | Windows 단일 실행 포터블 (x64) | 73MB |
| `AXGATE-Log-Viewer-0.1.2-win-x64.zip` | Windows 폴더 포터블 x64 (`AXGATE-Log-Viewer.exe`) | 101MB |
| `AXGATE-Log-Viewer-Setup-0.1.2-win-arm64.exe` | Windows 설치 프로그램 (ARM64) | 65MB |
| `AXGATE-Log-Viewer-0.1.2-win-arm64.zip` | Windows ARM 포터블 (zip 해제 후 `AXGATE-Log-Viewer.exe`) | 94MB |
| `AXGATE-Log-Viewer-0.1.2-mac-arm64.dmg` | macOS 설치 (Apple Silicon) | 85MB |
| `AXGATE-Log-Viewer-0.1.0-mac-arm64.dmg` | macOS 설치 (Apple Silicon, 이전) | 86MB |
| `AXGATE-Log-Viewer-0.1.0-mac-x64.dmg` | macOS 설치 (Intel) | 90MB |
...
- 자동 테스트 23개 통과 (IP 디코딩, CSV 판별, 필터 SQL, 샘플 `.adb`/`.csv` 적재)
...
| 항목 | 이유 |
|---|---|
| **저장된 필터 프리셋** | `PLAN2` 2.4절이 명시적으로 4.1절 제안으로 미루라고 적혀 있다. 저장 위치(앱 전용 폴더 vs 브라우저 localStorage), 이름 규칙, 종류 간 프리셋 공유 여부를 아직 확정하지 않았다. |
| **타임라인 드래그로 구간 선택** | 막대 그래프와 **클릭 → 해당 버킷 기간 필터**는 넣었다. 드래그(브러시)는 마우스 제스처·접근성·모바일 폭 처리가 있어, 클릭만으로도 기간 칩으로 해제/조정이 되므로 1차에서는 뺐다. |
...
| **최근 워크스페이스 목록** | 브라우저 파일 선택/드래그는 **폴더 경로를 안정적으로 다시 열 수 없다**. Electron 네이티브 다이얼로그가 있을 때 의미가 있다. 지금은 웹으로 테스트하는 단계라 목록이  vis할 경로가 없다. |
| **최초 실행 온보딩 투어** | 화면 좌표 기반 오버레이는 레이아웃이 필터 바·검색 결과처럼 자주 바뀌면 깨지기 쉽다. 시작 화면 안내 문구로 1차는 충분하다고 봤다. |
| **OS 파일 연결 (`.adb` 더블클릭)** | electron-builder `fileAssociations` + `open-file` / `argv` 처리가 필요하다. 설치 파일은 만들어졌지만 확장자 연결은 아직 넣지 않았다. CSV는 문서대로 연결에서 제외할 예정. |
...
| **네트워크 호출 린트/CI** | 오프라인 원칙은 시작 화면·설정 문구와, 외부 API를 쓰지 않는 구조로 반영했다. 빌드 파이프라인 grep은 패키징/CI를 만들 때 같이 넣는 것이 맞다. |
...
- 타임라인은 클릭 선택만 지원한다 (드래그 구간 선택 없음).
- 웹에서 연 폴더는 OS 경로가 없어 “최근 항목으로 다시 열기”가 없다.
...
1. Windows `.msi` (Windows 머신에서 빌드) / 커스텀 앱 아이콘
2. 타임라인 드래그 구간 선택
3. 저장된 필터 프리셋 (패키징 후 앱 데이터 폴더)
